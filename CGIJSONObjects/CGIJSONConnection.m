...
- (void)sendInvocation:(CGIJSONInvocation *)invocation
{
    ...
    err = nil;
    if ([self shouldFailWithData:responseBody response:response error:&err])
    {
        [self didFailWithError:err];
        return;
    }
...
{
    if ([self.delegate respondsToSelector:@selector(connection:shouldFailWithData:response:error:)])
        return [self.delegate connection:self shouldFailWithData:data response:response error:error];
    return NO;
}
